...
public class AdminController {
    ...
    public void initialize() {
        if (Admin.getAllApplicants().isEmpty()) {
            noApplicantsLabel.setVisible(true);
        } else {
            ObservableList<String> users = FXCollections.observableArrayList(Admin.getAllApplicants().keySet());
    ...
    private void handleUserSelection() {
        ...
        if (selectedName != null) {
            ...
            String phoneNumber = Admin.getApplicant(selectedName).getPhoneNumber();
            phoneNumberLabel.setText("Phone Number: " + phoneNumber);
            String skills = String.join(", ", Admin.getApplicant(selectedName).getSkills());
            skillsLabel.setText("Skills: " + skills);
            String companies = String.join(", ", Admin.getApplicant(selectedName).getCompanies());
